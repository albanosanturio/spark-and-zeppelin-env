...
2. Create and parse a config file with queries and settings for each table
3. Create the table structure (using the settings from the config file) in the local database
4. Given a path, load the tables with all the data inside that path (script goes through all files in that directory)

Methods:
Object CreateExternalTable
Method CreateExternalTable.createExternalTableStruct (creates schema)
...
// Regular expressions to find the partition field and value in the path
// TODO: Regular expression to find the partition when is execution_date; { val exDateExp = new Regex("") }
...
object CreateExternalTable {
   ...
   */
  def loadDataToExternalTableYM(database: String, tableName: String, inputFilePath: String,
   ...
   *
   * @param path local path where the files are
   */

  // This method works for partition = file_month + file_year
  // TODO method for partition = execution_date
        ...
        f => {
            f.isDirectory match {
                ...
                case false =>{
                    ...
                    CreateExternalTable.loadDataToExternalTableYM(
                        database = "avangrid",
