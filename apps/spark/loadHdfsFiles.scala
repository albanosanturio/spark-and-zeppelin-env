...
/** Factory for createing external tables */
object CreateExternalTable {
  ...
  def createExternalTableStruct(database: String, tableName: String): Unit = {
    ...
    val query = queries(queryName).toString().replace("${table_name}", tableName).replace("${database}",database)
  ...
  /** Creates and loads all the files in a path in Azure Data Lake Storage Gen2
   *
        ...
        f.isDir match {
          ...
          case false => {
            ...
            loadDataToExternalTable(
              ...
              partition_column_0 = part0.group(1),
              partition_value_0 = part0.group(2),
              partition_column_1 = part1.group(1),
              partition_value_1 = part1.group(2)
            )
          }
        }
      }
...
/opt/bitnami/spark/bin/spark-shell -i /opt/spark-apps/test-create-tables.scala